...
variable "github_token" {
  description = "GitHub token. Fine-grained PAT scoped to cqvo/wumbotechsite with Administration + Issues + Variables + Secrets (read/write) and Metadata (read); or a classic PAT with 'repo' scope. See README. Provide via TF_VAR_github_token."
  type        = string
  sensitive   = true
}
